...
| typ_ext_nil: 
    typ_ext Fsub.typ_rcd_nil typ_rcd_nil
| typ_ext_cons: forall i (T1 T2 : Fsub.typ) T1' T2',
    typ_ext T1 T1' -> typ_ext T2 T2' -> 
    typ_ext (Fsub.typ_rcd_cons i T1 T2) (typ_rcd_cons i T1' T2')
.
...
| exp_ext_proj: forall i e e',
    exp_ext e e' ->
    exp_ext (Fsub.exp_rcd_proj e i) (exp_rcd_proj e' i)
| exp_ext_rcd_nil: exp_ext Fsub.exp_rcd_nil exp_rcd_nil
| exp_ext_rcd_cons: forall i e1 e2 e1' e2',
    exp_ext e1 e1' ->
    exp_ext e2 e2' ->
    exp_ext (Fsub.exp_rcd_cons i e1 e2) (exp_rcd_cons i e1' e2')
.
...
Ltac inv_rt :=
  try (solve
   [ repeat match goal with
          | H:Fsub.rt_type ?T |- _ => inversion H; clear H
...
Lemma collectLabel_conserv : forall { T T' },
  ExtDefs.typ_ext T T' -> Fsub.collectLabel T = collectLabel T'.
Proof with auto.
  intros.
  induction H...
  simpl. rewrite IHtyp_ext2...
Qed.


Lemma rt_type_conserv : forall { T T' },
  ExtDefs.typ_ext T T' -> Fsub.rt_type T <-> rt_type T'.
Proof with auto.
  intros.
  induction H;try solve [
    split;intro C;try inversion C
  ]...
  - split;intros...
  - split;intros...
Qed.
...
Lemma rt_expr_conserv : forall { e e' },
  ExtDefs.exp_ext e e' -> Fsub.rt_expr e <-> rt_expr e'.
Proof with auto.
  intros.
  induction H;try solve [
    split;intro C;try inversion C
...
Lemma collectLabele_conserv : forall { e e' },
ExtDefs.exp_ext e e' -> Fsub.collectLabele e = collectLabele e'.
Proof with auto.
  intros.
  induction H...
  simpl. rewrite IHexp_ext2...
Qed.


Lemma Tlookup_conserv2: forall {i A A' t},
  ExtDefs.typ_ext A A' ->
  Fsub.Tlookup i A = Some t ->
  exists t', Tlookup i A' = Some t' /\ ExtDefs.typ_ext t t'.
Proof with auto.
  intros.
  induction H;try solve [inv H0].
  - simpl in *. destruct (i0==i).
    + inv H0. exists T1'. split...
    + destruct (IHtyp_ext2 H0). destruct_hypos.
      exists x. split...
Qed. 


Lemma Tlookup_conserv: forall {i A A' t'},
  ExtDefs.typ_ext A A' ->
  Tlookup i A' = Some t' ->
  exists t, Fsub.Tlookup i A = Some t /\ ExtDefs.typ_ext t t'.
Proof with auto.
  intros.
  induction H;try solve [inv H0].
  - simpl in *. destruct (i0==i).
...
Proof with auto.
  ...
  - inv H3... apply WF_all with (L:=L)...
    ...
    { apply open_tt_conserv... }
  - inv H0...
  - inv H2... apply WF_rcd_cons...
    + apply rt_type_conserv in H8... tauto.
    + apply collectLabel_conserv in H8... rewrite <- H8...
Qed.
...
Proof with auto.
  ...
  - apply sa_rcd...
    + eapply wf_env_conserv;eassumption...
    + apply rt_type_conserv in H8... tauto.
    + apply rt_type_conserv in H7... tauto.
    + apply collectLabel_conserv in H8...
      apply collectLabel_conserv in H7...
      rewrite <- H8, <- H7...
    + eapply wf_conserv with (T:=A1); eassumption...
    + eapply wf_conserv with (T:=A2); eassumption...
    + intros. 
      destruct (Tlookup_conserv H8 H10).
      destruct (Tlookup_conserv H7 H11). destruct_hypos.
      eapply H6 with (i:=i) (t1:=x) (t2:=x0)...
Qed.
...
Proof with auto.
  ...
  - destruct (ExtDefs.typ_ext_ex S) as [S' ?].
    ...
    eapply sub_conserv;try eassumption...
  - destruct (ExtDefs.typ_ext_ex T) as [T' ?].
    inv H0.
    epose proof Tlookup_conserv2 H4 H.
    destruct H5 as [Ti']. destruct_hypos.
    rewrite (ExtDefs.typ_ext_inj1 H1 H6) in *.
    apply typing_proj with (T:=T')...
  - inv H1. inv H0. constructor.
...
Proof with auto.
  ...
  - apply Fsub.sa_rcd...
    ...
    + eapply wf_conserv with (T':=A2); eassumption...
    + intros. 
      destruct (Tlookup_conserv2 H8 H10).
      destruct (Tlookup_conserv2 H7 H11). destruct_hypos.
      eapply H6 with (i:=i) (t1:=x) (t2:=x0)...
Qed.
...
Proof with auto.
  ...
  - inv H0.
  - inv H.
    + inv H0. exists Fsub.typ_rcd_nil...
    + inv H0. exists (Fsub.typ_rcd_cons i T0 T3)...
Qed.
...
Proof with auto.
  ...
  - inv H0.
    ...
    epose proof Tlookup_conserv H4 H.
  ...
  - inv H0.
    destruct IHtyping with (e0:=e1) (E0:=E0) as [Trcd' [? ?]]... inv H3.
    (* destruct (exposure_ext_ex2 H3 H5 H) as [Trcd'' ?]. *)
    exists Fsub.typ_bot. split...
  ...
  - inv H0. exists Fsub.typ_rcd_nil. split...
