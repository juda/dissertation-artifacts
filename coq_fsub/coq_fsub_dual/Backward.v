...
Proof with auto.
  ...
  -
    ...
    +
      rewrite <- subst_label_open_tt_var.
      rewrite_env (map (drop_label X) (X0 ~ bind_sub typ_bot ++ E1) ++ (X~ bind_sub U) ++ E2).
      apply H0...
      rewrite_env (X0 ~ bind_sub typ_bot ++ E1 ++ (X, bind_sub U) :: E2)...
    +
...
Lemma wf_env_drop_label : forall E1 E2 X,
    ...
    wf_env (map (drop_label X) E1++ [(X, bind_sub typ_bot)] ++ E2).
Proof with auto.
  induction E1;intros...
  destruct a.
...
Lemma open_twice_to_one: forall E1 E2 A B X,
    ...
    X \notin fl_env E2 ->
    sub (map (drop_label X) E1++ [(X, bind_sub typ_bot)] ++ E2) (subst_label X A) (subst_label X B).
Proof with eauto. 
  intros.
  assert (wf_env (E1++ [(X, bind_sub typ_bot)] ++ E2)) as HE.
  get_well_form...
  ...
  -
    ...
    rewrite_env (map (drop_label X) (X0 ~ bind_sub S2 ++ E1) ++ (X~ bind_sub typ_bot) ++ E2).
    rewrite <- subst_label_open_tt_var.
    rewrite <- subst_label_open_tt_var.
    apply H2...
    rewrite_env (X0 ~ bind_sub S2 ++ E1 ++ (X, bind_sub typ_bot) :: E2)...
    constructor...
    get_well_form...
  -
    ...
    +
      rewrite <- subst_label_open_tt_var.
      rewrite_env (map (drop_label X) (X0 ~ bind_sub typ_bot ++ E1) ++ (X~ bind_sub typ_bot) ++ E2).
      apply WF_drop_label...
      apply H...
...
Lemma sub_nominal_inversion: forall E X A B,
    sub (X~bind_sub typ_bot ++ E) (open_tt A (typ_label X (open_tt A X))) (open_tt B (typ_label X (open_tt B X))) ->
    X \notin  fl_tt A \u fl_tt B \u fl_env E ->
    sub (X~bind_sub typ_bot ++ E) (open_tt A X) (open_tt B X).
Proof with auto.
  intros.
  rewrite_env (nil ++ X~bind_sub typ_bot ++ E) in H.
...
Proof with eauto.
  ...
  -
    apply WF_rec with (L:=L \u {{X}} \u dom E1 \u dom E2);intros...
    rewrite_env ((X0 ~ bind_sub typ_bot ++ E1) ++ E2).
...
Proof with eauto.
  ...
  -
    ...
    +
      ...
      rewrite_env (((X0, bind_sub typ_bot) :: E1) ++ (X, U) :: E2) in H.
      apply WF_strengthening in H...
      solve_notin.
      rewrite_env (X0 ~ bind_sub typ_bot ++ E1 ++ E2)...
    +
      specialize_x_and_L X0 L.
      rewrite_env (((X0, bind_sub typ_bot) :: E1) ++ (X, U) :: E2) in H0.
      apply WF_strengthening in H0...
      solve_notin.
      rewrite_env (X0 ~ bind_sub typ_bot ++ E1 ++ E2)...
    +
