...
Inductive exposure : env -> typ -> typ -> Prop :=
  ...
  | exposure_bot : forall E,
      exposure E typ_bot (typ_mu typ_bot)
...
Inductive typing : env -> exp -> typ -> Prop :=
  ...
  | typing_app_bot : forall E e1 e2 T2,
      typing E e1 typ_bot ->
      typing E e2 T2 ->
  ...
  | typing_tapp_bot : forall E e1 T2,
      typing E e1 typ_bot -> WF E T2 ->
  ...
  | typing_fold : forall G A S e T,
     ...
     typing G (exp_fold T
      ...
      T
      (* typ_mu A) *)
 (* | typing_fold_bot : forall G A S e,
      typing G e S ->
      WF G (typ_mu A) ->
      sub G S (open_tt A typ_bot) ->
      typing G (exp_fold typ_bot
        (* (typ_mu A) conflicting choice of annotation *)
        e) typ_bot *)
 | typing_unfold : forall G A A' T e,
  ...
  | typing_proj_bot : forall E e i,
      typing E e typ_bot ->
      typing E (exp_rcd_proj e i) typ_bot
...
Proof with auto.
  ...
  - eapply sa_trans_tvar with (U:=U)...
    apply IHexposure2... apply WF_from_binds_typ with (x:=X)...
Qed.
...
Lemma exposure2_ex: forall E A,
  ...
  exposure2 E B A /\ not_var_typ B /\ WF E B.
Proof with auto.
  intros. revert A H0.
  induction E;intros...
  {
...
Proof with auto.
  ...
  - apply typing_app_bot with (T2:=T2)...
  ...
  - apply typing_tapp_bot with (T2:=T2)...
    + apply WF_weakening... apply wf_typ_strengthening in H0...
...
Proof with auto.
  ...
  - simpl. apply typing_app with (T11:= subst_tt X Y T11) (T2:=subst_tt X Y T2) (T1:=subst_tt X Y T1)...
    ...
    + apply IHtyping2... 
    + apply sub_replacing...
  - simpl. apply typing_app_bot with  (T2:=subst_tt X Y T2)...
    + apply IHtyping1...
    + apply IHtyping2... 
  - simpl. apply typing_tabs with (L:=L \u {{ X }}
  \u dom (E1 ++ Y ~ bind_typ U ++ E2) ).
  ...
  - simpl. rewrite subst_tt_open_tt...
    ...
    + apply sub_replacing...
  - simpl.
    apply typing_tapp_bot with (T2:=subst_tt X Y T2)...
    + apply IHtyping...
    + apply WF_replacing with (Y:=Y) in H0... 
...
(* Lemma fold_typ_det: forall E e T U, typing E (exp_fold T e) U ->
  Algo.typing E (exp_fold T e) T /\ sub E T U.
Proof with auto.
  intros.
  dependent induction H.
  - split...
    + apply Algo.typing_fold with (A:=A) (S:=T)...
    + get_well_form. apply Reflexivity...
  - specialize (IHtyping _ _ eq_refl). destruct_hypos.
    split... apply sub_transitivity with (Q:=S)...
Qed. *)
...
Proof with auto.
  ...
  -
    ...
    destruct (sub_inv_arrow H2).
    { subst... exists typ_bot. split...
      + apply Algo.typing_app_bot with (T2:=T2')...
      + get_well_form... constructor... inv H7... }
    destruct H8 as [N11 [N12 ?]]. destruct_hypos.
    exists N12. split...
  ...
  -
    ...
    destruct (sub_inv_all H2).
    { subst. exists typ_bot. split...
      + apply Algo.typing_tapp_bot... get_well_form...
      + constructor... apply wf_open_all with (A:=T1)...
        get_well_form...
    }
    destruct H6 as [N11 [L ]]. destruct_hypos.
    destruct H6.
    inversion H6;subst;inv_rt. { inversion H8;inv_rt. }
 ...
 *)
    ...
    + eapply Algo.typing_fold with (S:=S) (A:=A2)...
      { apply sub_transitivity with (Q:=open_tt A T)...
        ...
        apply sub_through_subst_tt with (Q:=typ_bot)...
      }
    + apply Reflexivity...
  ...
  -
    ...
    { exists (open_tt typ_bot typ_bot). split...
      +  apply Algo.typing_unfold with (A':=S)...
      + unfold open_tt at 1. simpl.
        constructor... apply wf_open_rec2... }
    exists (open_tt A1 (typ_mu A1)).
    split...
    { 
    ...
    { 
      pick_fresh Y.
      specialize_x_and_L Y L.
      apply sub_nominal_inversion in H2...
      rewrite subst_tt_intro with (X:=Y)...
      rewrite subst_tt_intro with (X:=Y) (U:=(typ_mu A1))...
      rewrite_env (map (subst_tb Y (typ_mu A1)) nil ++ E).
      apply sub_through_subst_tt with (Q:=typ_bot)... }
  - 
    destruct IHtyping as [S']. destruct_hypos.
    exists S'. split...
  ...
  -
    ...
    destruct (rcd_types_match _ H2 H0).
    2:{ subst. exists typ_bot. split...
        + constructor... apply wf_rcd_lookup with (E:=G) in H0... }
    destruct H6 as [Ti']. destruct_hypos.
    exists Ti'. split...
    apply Algo.typing_proj with (T:=S')...
  -
  ...
  -
    ...
    { dependent destruction H7;inv_rt...
      { apply Algo.bot_uninhabited_rt  in H6... destruct H6. }
    }
...
Proof with auto.
  ...
  - replace (typ_bot) with (open_tt typ_bot T2).
    ...
    apply typing_tapp with (T1:=typ_bot)...
    + apply typing_sub with (S:=typ_bot)...
      apply Algo.typing_regular in H. destruct_hypos.
  ...
  - apply typing_proj with (T:=T)...
  - apply typing_proj with (T:=(typ_rcd_cons i typ_bot typ_rcd_nil))...
    + apply typing_sub with (S:=typ_bot)...
      constructor... 
      apply Algo.typing_regular in H. destruct_hypos...
