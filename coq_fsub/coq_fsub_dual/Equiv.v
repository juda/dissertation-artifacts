  ...
  -
    ...
    +
      ...
      *
        ...
        apply subst_tt_wf...
        get_well_form.
        rewrite_env (nil ++ X0 ~ bind_sub typ_bot ++ E).
        apply WF_weakening...
      *
        rewrite subst_tt_open_tt_var...
        apply subst_tt_wf...
...
Lemma WF_narrowing_env_subst: forall E1 E2 X T S,
    WF (E1 ++ X ~ bind_sub typ_bot ++ E2) T ->
    wf_env (E1 ++ X ~ bind_sub typ_bot ++ E2) ->
    WF (map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2) T.
Proof with auto.
  intros.
  assert (type T).
  get_type...
  ...
  -
    ...
    +
      rewrite_env (map (subst_tb X S) (X0 ~ bind_sub typ_bot ++ E1) ++ X ~ bind_sub typ_bot ++ E2).
      apply H2...
      apply H3...
      constructor...
    +
  ...
  -
    ...
    rewrite_env (nil ++ X0 ~ bind_sub T1 ++ map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2).
    apply WF_narrowing with (V:=subst_tt X S T1).
    rewrite_env (map (subst_tb X S) (X0 ~ bind_sub T1 ++ E1) ++ X ~ bind_sub typ_bot ++ E2).
    apply H0...
    apply H3...
    constructor...
  -
...
Lemma equiv_sub_subst_refl: forall A X E1 E2 C D S,
    ...
    wf_env (E1 ++ X ~ bind_sub typ_bot ++ E2) ->
    equiv (X ~ bind_sub typ_bot ++ E2) C D ->
    equiv (X ~ bind_sub typ_bot ++ E2) C S ->
    equiv (X ~ bind_sub typ_bot ++ E2) S D ->
    sub (map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2) (subst_tt X C A) (subst_tt X D A).
Proof with auto.
  intros.
  unfold equiv in *.
  destruct_hypos.
  ...
  -
    destruct (X==X0);subst...
    +
      rewrite_env (nil ++ map (subst_tb X0 S) E1 ++ (X0, bind_sub typ_bot) :: E2).
      apply Sub_weakening...
      apply wf_env_map_free...
      get_well_form...
    +
  ...
  -
    ...
    +
      ...
      rewrite_env (map (subst_tb X S) (X0~bind_sub typ_bot ++ E1) ++ (X, bind_sub typ_bot) :: E2).
      apply subst_tt_wf...
      rewrite_env (nil ++ map (subst_tb X S) (X0 ~ bind_sub typ_bot ++ E1) ++ (X~ bind_sub typ_bot) ++ E2).
      apply WF_weakening...
      get_well_form...
  ...
  -
    ...
    +
      rewrite subst_tt_open_tt_var...
      rewrite subst_tt_open_tt_var...
      rewrite_env ( nil ++ X0 ~ bind_sub (subst_tt X D T1) ++ map (subst_tb X S) E1 ++ (X, bind_sub typ_bot) :: E2).
      ...
      *
        ...
        get_well_form;apply Reflexivity...
        unfold equiv;rewrite_env (nil ++ map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2);
  ...
  -
    inversion H0;subst.
    rewrite_env (map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2).
    apply sa_rcd...
    * apply wf_env_map_free... get_well_form...
    * simpl. intro x. intros.
      apply union_iff in H8. destruct H8...
      apply subst_tt_collect3 with (E:=(E1 ++ X ~ bind_sub typ_bot ++ E2)) in H8 ...
      apply union_iff. right. 
      apply subst_tt_collect2 with (E:=(E1 ++ X ~ bind_sub typ_bot ++ E2))...
    * get_well_form. constructor...
      { apply subst_tt_wf...
...
Proof with auto.
  ...
  -
    constructor...
    apply wf_env_map_free2 with (B:=bind_sub typ_bot)...
    get_well_form...
  -
    get_well_form...
    constructor...    
  ...
  -
    ...
    +
      rewrite subst_tt_open_tt_var...
      rewrite_env (map (subst_tb X S) (X0~bind_sub typ_bot ++ E1) ++  E2).
      apply subst_tb_wf3...
      apply WF_narrowing with (V:=typ_bot)...
      apply H9...
      get_well_form...
    +
      rewrite subst_tt_open_tt_var...
  ...
  -
    ...
    +
      ...
      *
        ...
        rewrite_env (map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2).
        apply wf_env_map_free...
        rewrite_env (nil ++ X ~ bind_sub typ_bot ++ E2).
        apply WF_weakening...
        get_well_form...
        rewrite_env (map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2).
        apply wf_env_map_free...
  ...
  -
    ...
    constructor...
  -
    apply Reflexivity... apply wf_env_map_free2 with (B:=bind_sub typ_bot)...
    get_well_form...
  -
    inversion H0;subst.
    apply sa_rcd...
    * apply wf_env_map_free2 with (B:=bind_sub typ_bot)... get_well_form...
    * simpl. intro x. intros.
      apply union_iff in H8. destruct H8...
      apply subst_tt_collect3 with (E:=(E1 ++ X ~ bind_sub typ_bot ++ E2)) in H8 ...
      apply union_iff. right. 
      apply subst_tt_collect2 with (E:=(E1 ++ X ~ bind_sub typ_bot ++ E2))...
    * get_well_form. constructor...
      { apply subst_tb_wf3... apply WF_narrowing with (V:=typ_bot)... }
      { apply subst_tb_wf3... apply WF_narrowing with (V:=typ_bot)... }
      { apply subst_tt_rt_type with (E:=E1 ++ X ~ bind_sub typ_bot ++ E2)...
...
Lemma equiv_sub_subst_not_var: forall A B X E1 E2 C D S,
    sub (E1 ++ X ~ bind_sub typ_bot ++ E2) A B ->
    equiv E2 C D ->
    equiv E2 C S ->
    equiv E2 S D ->
...
Proof with auto.
  ...
  -
    ...
    +
      ...
      *
        apply binds_map_free_sub with (Q:=typ_bot)...
        get_well_form...
      *
        apply IHsub...
        get_well_form.
  ...
  -
    ...
    +
      ...
      rewrite_env (map (subst_tb X S) (X0 ~ bind_sub typ_bot ++ E1) ++ E2).
      apply subst_tb_wf3...
      rewrite_env ((X0 ~ bind_sub typ_bot ++ E1) ++ X ~ bind_sub S ++ E2).
      apply WF_narrowing with (V:=typ_bot)...
      apply H...
      get_well_form...
    +
  ...
  -
    ...
    + apply subst_tt_rt_type with (E:=E1 ++ X ~ bind_sub typ_bot ++ E2)...
      apply WF_weakening. add_nil. apply WF_weakening. get_well_form...
    + simpl. intro x. intros.
      apply subst_tt_collect3 with (E:=(E1 ++ X ~ bind_sub typ_bot ++ E2)) in H15...
      apply subst_tt_collect2 with (E:=(E1 ++ X ~ bind_sub typ_bot ++ E2))...
    + apply subst_tb_wf3... apply WF_narrowing with (V:=typ_bot)...
      get_well_form...
    + apply subst_tb_wf3... apply WF_narrowing with (V:=typ_bot)...
      get_well_form...
    + intros.
      apply Tlookup_subst with (E:=E1 ++ X ~ bind_sub typ_bot ++ E2)in H15...
      apply Tlookup_subst with (E:=E1 ++ X ~ bind_sub typ_bot ++ E2)in H16...
...
Lemma equiv_sub_subst: forall A B X E1 E2 C D S,
    ...
    equiv (X ~ bind_sub typ_bot ++ E2) C S ->
    equiv (X ~ bind_sub typ_bot ++ E2) S D ->
    sub (map (subst_tb X S) E1 ++ X ~ bind_sub typ_bot ++ E2) (subst_tt X C A) (subst_tt X D B).
Proof with auto.
  intros.
  unfold equiv in *.
  destruct_hypos.
  ...
  -
    destruct (X0==X);subst...
    +
      rewrite_env (nil ++ map (subst_tb X S) E1 ++ (X, bind_sub typ_bot) :: E2).
      apply Sub_weakening...
      apply wf_env_map_free...
      get_well_form...
    +
  ...
  -
    ...
    +
      ...
      rewrite subst_tt_open_tt_var...
      rewrite subst_tt_open_tt_var...
      rewrite_env (nil ++ X0 ~ bind_sub (subst_tt X D S2) ++ map (subst_tb X S) E1 ++ (X, bind_sub typ_bot) :: E2).
      apply sub_narrowing with (Q:=subst_tt X S S2).
      *
        apply equiv_sub_subst_refl;try solve [get_well_form;auto|unfold equiv;auto]...
        unfold equiv;get_well_form;split;apply Reflexivity...
      *
        rewrite_env  (map (subst_tb X S) (X0 ~ bind_sub S2 ++ E1) ++ (X, bind_sub typ_bot) :: E2).
        apply H2...
  -
    assert (type C) by (get_type;auto).
    assert (type D) by (get_type;auto).
    ...
    +
      rewrite subst_tt_open_tt_var...
      apply subst_tt_wf...
      rewrite_env (nil ++ (X0 ~ bind_sub typ_bot ++ map (subst_tb X S) E1) ++ (X, bind_sub typ_bot) :: E2).
      apply WF_weakening...
      get_well_form...
      rewrite_env (map (subst_tb X S) (X0~bind_sub typ_bot++E1) ++ (X, bind_sub typ_bot) :: E2).
      apply WF_narrowing_env_subst...
  ...
  -
    ...
    rewrite_env  (map (subst_tb X S) E1 ++ (X ~ bind_sub typ_bot) ++ E2).
    apply sa_rcd...
    + apply wf_env_map_free with (B:=bind_sub typ_bot)... 
      get_well_form...
    + apply subst_tt_rt_type with (E:=E1 ++ X ~ bind_sub typ_bot ++ E2)...
...
Lemma equiv_unfolding: forall E C D X,
    ...
    equiv (X~bind_sub typ_bot ++ E) (typ_label X (open_tt C X)) (typ_label X (open_tt D X)).
Proof with auto.
  intros.
  unfold equiv in *.
  destruct_hypos.
...
Lemma equiv_sub_unfolding: forall C D Y E,
      sub (Y ~ bind_sub typ_bot ++ E) (open_tt C Y) (open_tt D Y) ->
      sub (Y ~ bind_sub typ_bot ++ E) (open_tt D Y) (open_tt C Y) ->
...
Proof with auto.
  ...
  -
    ...
    +
      ...
      rewrite subst_tt_intro with (X:=Y)...
      rewrite_env (map (subst_tb Y (typ_label Y (open_tt C Y))) nil ++ Y ~ bind_sub typ_bot ++ E).
      subst.
