...
Lemma subst_tt_wf_env_label_rec: forall E1 X E2 U,
    wf_env (map (subst_tb X (typ_label X (open_tt U X))) E1 ++ (X, bind_sub typ_bot) :: E2) ->
...
Lemma binds_map_free_env2: forall E1 E2 X Y U S,
    ...
    binds X (bind_sub U) (E1 ++ (Y, bind_sub typ_bot) :: E2) ->
...
Proof with eauto.
  ...
  -
    ...
    +
      rewrite_env ((X0 ~ bind_sub typ_bot ++ E) ++ (X, bind_sub Q) :: E2).
...
(* Lemma subst_tb_wf_env_reverse: forall E E2 X T,
    wf_env (map (subst_tb X T) E ++ (X~ bind_sub typ_bot) ++ E2) ->
    WF ((X~ bind_sub typ_bot) ++ E2) T ->
    wf_env (E ++ (X~ bind_sub typ_bot) ++ E2).
Proof with auto.
  induction E;intros...
  destruct a.
  destruct b.
...
Lemma sub_generalize_intensive : forall E1 E2 A B C D X m S,
    ...
    sub (map (subst_tb X (typ_label X (open_tt S X))) E1 ++ X ~ bind_sub typ_bot ++ E2) (subst_tt X (typ_label X (open_tt C X)) A) (subst_tt X (typ_label X  (open_tt D X)) B) ->
...
Proof with auto.
  ...
  -
    ...
    +
      ...
      destruct n...
      rewrite H5.
      apply subst_tb_wf with (Q:=bind_sub typ_bot)...
      destruct m;simpl in *;get_well_form...
  -
    simpl.
    constructor...
    ...
    +
      ...
      *
        ...
        --
          ...
          ++
            solve_notin.
            apply notin_fl_env with (X:=X0) (E:=E1 ++ (X, bind_sub typ_bot) :: E2)...
            solve_notin.
          ++
  ...
  -
    simpl in *.
    dependent destruction H4.
    assert (equiv (map (subst_tb X (typ_label X (open_tt S X))) E1 ++ (X, bind_sub typ_bot) :: E2)
        ...
        --
          ...
          ++
            ...
            **
              get_well_form...
            **
              apply equiv_unfolding... destruct H10. split...
            **
              apply equiv_unfolding... destruct H10. split...
              (* apply equiv_reflexivity;get_well_form...
              destruct m;simpl in *... *)
            **
              apply equiv_unfolding... apply equiv_reflexivity;get_well_form...
              destruct m;simpl in *...
            **
              apply H4...
            **
  ...
  -
    ...
    +
      rewrite subst_tt_open_tt_var...
      rewrite_env (nil ++ map (subst_tb X (typ_mu S)) (X0~bind_sub typ_bot ++ E1) ++ E2).
      apply WF_narrowing_env with (S:=typ_mu C)...
      apply subst_tb_wf with (Q:=bind_sub typ_bot)...
      apply H...
    ...
    +
      ...
      destruct_hypos.
      rename X0 into Y.
      rewrite_env (map (subst_tb X (typ_mu S)) (Y~bind_sub typ_bot ++ E1) ++ E2).
  ...
  -
    assert (Hwf1: WF (map (subst_tb X (typ_mu S)) E1 ++ E2) (subst_tt X (typ_mu C) A1)).
    {
      ...
      apply WF_narrowing_env with (S:=typ_mu C)...
      eapply subst_tb_wf with (Q:=bind_sub typ_bot)... destruct m...
    }
    ...
    apply sa_rcd...
    + get_well_form... apply wf_env_map_free2 with (B:=bind_sub typ_bot)...
      destruct m...
    + get_well_form.
    ...
    + get_well_form.
      destruct m;apply Infrastructure.subst_tt_rt_type;get_type...
    + 
      apply  label_equiv_reserve with (E:=(E1 ++ [(X, bind_sub typ_bot)] ++ E2))...
      simpl. 
      apply AtomSetProperties.equal_refl...
    +
      intros.
      apply Tlookup_subst with (E:=(E1 ++ (X, bind_sub typ_bot) :: E2)) in H11...
      apply Tlookup_subst with (E:=(E1 ++ (X, bind_sub typ_bot) :: E2)) in H12...
      destruct H11 as [t1'].
      destruct H12 as [t2'].
      ...
      simpl in *.
      apply H6 with (m:=m)...
      { apply lookup_some_in_fl_tt with (E:=(E1 ++ (X, bind_sub typ_bot) :: E2)) in H16...
        apply lookup_some_in_fl_tt with (E:=(E1 ++ (X, bind_sub typ_bot) :: E2)) in H14...
        solve_notin.
...
Lemma sub_generalize_unfolding : forall E1 E2 A B C D X  S,
    ...
    (sub (map (subst_tb X (typ_label X (open_tt S X))) E1 ++ X ~ bind_sub typ_bot ++ E2) (subst_tt X (typ_label X  (open_tt C X)) A) (subst_tt X (typ_label X (open_tt D X)) B) ->
     sub (map (subst_tb X (typ_mu S)) E1 ++ E2) (subst_tt X  (typ_mu C) A) (subst_tt X (typ_mu D) B)) /\ (sub (map (subst_tb X (typ_label X (open_tt S X))) E1 ++ X ~ bind_sub typ_bot ++ E2) (subst_tt X (typ_label X  (open_tt D X)) A) (subst_tt X (typ_label X (open_tt C X)) B) ->
