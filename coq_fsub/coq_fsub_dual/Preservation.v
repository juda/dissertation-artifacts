...
Proof with eauto.
  ...
  -
    ...
    rewrite_alist (([(X, bind_sub typ_bot)] ++ F) ++ E).
...
Proof with auto.
  ...
  2:{ apply sub_transitivity with (Q:=open_tt C (typ_mu C))...
      { apply unfolding_lemma... }
        ...
        assert ( forall X : atom,
          X `notin` L \u (fv_tt C)->
          sub (X ~ bind_sub typ_bot ++ empty) (open_tt C X)
            (open_tt D X)
        ).
        { intros. pick_fresh X0. specialize_x_and_L X0 L.
          apply sub_nominal_inversion in H7...
          rewrite_env (empty ++ X ~ bind_sub typ_bot ++ empty).
  ...
  assert ( forall X : atom,
    X `notin` L \u (fv_tt A)->
    sub (X ~ bind_sub typ_bot ++ empty) (open_tt A X)
      (open_tt B X)
  ).
  { intros. pick_fresh X0. specialize_x_and_L X0 L.
    apply sub_nominal_inversion in H7...
...
Proof with auto.
  ...
  -
    ...
    { inversion H3. }
    inversion H2;inv_rt;subst. { inversion H1;inv_rt. }
...
Lemma typing_inv_tabs : forall E S1 e1 T,
typing E (exp_tabs S1 e1) T ->
forall U1 U2 S, sub E T (typ_all U1 U2) -> sub E U1 S ->
...
Proof with auto.
  ...
  -
    ...
    destruct H1.
    2:{ subst. apply bot_uninhabited in H0... destruct H0. }
