...
Proof with eauto.
  ...
  -
    ...
    intros.
    rewrite_alist (([(X, bind_sub typ_bot)] ++ E1) ++ E ++ E2).
    apply H0...
    intros.
    rewrite_alist (([(X, bind_sub typ_bot)] ++ E1) ++ E ++ E2).
    apply H2...
...
Proof with eauto.
  ...
  -
    ...
    rewrite_alist (([(X0, bind_sub typ_bot)] ++ F) ++ [(X, bind_sub U)] ++ E)...
    eapply H0 with (V0:=V)...
    rewrite_alist (([(X0, bind_sub typ_bot)] ++ F) ++ [(X, bind_sub U)] ++ E)...
    eapply H2 with (V0:=V)...
Qed.
...
Proof with eauto.
  ...
  -
    ...
    rewrite subst_tt_open_tt_var...
    rewrite_env (map (subst_tb Z P) (X ~ bind_sub typ_bot ++ F) ++ E).
    apply H0 with (Q0:=Q)...
    apply WF_type in H3...
    rewrite <- subst_tt_open_tt_twice...
    rewrite_env (map (subst_tb Z P) (X ~ bind_sub typ_bot ++ F) ++ E).
  ...
  - 
    constructor...
    { apply subst_tt_rt_type with (E:=(F ++ (Z ~ Q) ++ E))...
      rewrite_env (nil ++ (F ++ Z~Q) ++ E)...
      apply WF_weakening... }
    { eapply subst_tt_collect... }
Qed.
...
Proof with eauto.
  ...
  -
    apply WF_rec with (L:=L \u {{Z}});intros...
    rewrite subst_tt_open_tt_var...
    rewrite_env (map (subst_tb Z P) (X ~ bind_sub typ_bot ++ F) ++ (Z,Q)::E).
    apply H0 with (Q0:=Q)...
    apply WF_type in H3...
    rewrite <- subst_tt_open_tt_twice...
    rewrite_env (map (subst_tb Z P) (X ~ bind_sub typ_bot ++ F) ++ (Z,Q)::E).
...
Proof with eauto.
  ...
  -
    apply WF_rec with (L:=L \u {{Z}});intros...
    rewrite subst_tt_open_tt_var...
    rewrite_env (map (subst_tb Z S) (X ~ bind_sub typ_bot ++ F) ++ E).
    apply H0 with (S0:=S)...
    apply WF_type in H3...
    rewrite <- subst_tt_open_tt_twice...
    rewrite_env (map (subst_tb Z S) (X ~ bind_sub typ_bot ++ F) ++ E).
  ...
  -
    constructor...
    { apply subst_tt_rt_type with (E:=(F ++ (Z ~ bind_sub S) ++ E))...
      rewrite_env (nil ++ (F ++ (Z~ bind_sub S)) ++ E)...
      apply WF_weakening... }
    { eapply subst_tt_collect... }
Qed.
...
Proof with auto.
  ...
  -
    ...
    rewrite subst_tt_open_tt_var...
    rewrite_env ((map (subst_tb X Y) (X0 ~ bind_sub typ_bot ++ E1)) ++ (Y, bind_sub T) :: E2).
    apply H0...
    solve_notin.
    rewrite <- subst_tt_open_tt_twice...
    rewrite_env ((map (subst_tb X Y) (X0 ~ bind_sub typ_bot ++ E1)) ++ (Y, bind_sub T) :: E2).
  ...
  -
    ...
    { apply subst_tt_rt_type' 
      with (E:=(E1 ++ (X ~ bind_sub T) ++ E2))... }
    { eapply subst_tt_collect... eauto. }
Qed.
