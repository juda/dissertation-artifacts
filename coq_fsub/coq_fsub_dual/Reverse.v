...
Lemma WF_narrowing_env_subst_inv: forall E1 E2 (X:atom) Y S,
    WF (map (subst_tb Y S) E1 ++ Y ~ bind_sub typ_bot ++ E2) X ->
    WF (E1 ++ Y ~ bind_sub typ_bot ++ E2) X.
Proof with auto.
  induction E1;intros...
  simpl in *.
  destruct a.
  ...
  -
    ...
    inversion BindsTacVal...
  -
    rewrite_env (nil ++ (a~ b) ++ E1 ++ (Y, bind_sub typ_bot) :: E2).
    apply WF_weakening...
    apply IHE1  with (S:=S)...
    apply WF_var with (U:=U)...
  -
    apply WF_var with (U:=typ_bot)...
  -
    apply WF_var with (U:=U)...
Qed.   
...
Proof with auto.
  ...
  -
    ...
    rewrite_env ((X0 ~ bind_sub typ_bot ++ E1) ++ map (subst_tb X T) E ++ E2). 
    eapply H0 with (S0:=S)...
    rewrite_env ((X0 ~ bind_sub typ_bot ++ E1) ++ map (subst_tb X T) E ++ E2). 
    eapply H2 with (S0:=S)...
Qed.
...
Lemma sub_map_inv_var: forall X Y X0 C E1 E2, 
    X <> Y -> X0 <> Y ->
  sub (map (subst_tb Y C) E1 ++ (Y, bind_sub typ_bot) :: E2) X X0 ->
  sub (map (subst_tb Y C) E1 ++ (Y, bind_sub typ_bot) :: E2) X0 X ->
  wf_env (E1 ++ (Y, bind_sub typ_bot) :: E2) ->
  sub (E1 ++ (Y, bind_sub typ_bot) :: E2) X X0.
Proof with auto.
  intros.
...
Lemma drop_label_reverse_wf: forall E1 E2 C D A X,
    WF (map (drop_label X) (map (subst_tb X (typ_label X C)) E1) ++
            (X, bind_sub typ_bot) :: E2)
       (subst_label X (subst_tt X (typ_label X D) A)) ->
    X \notin fl_tt A -> type D ->
    WF (E1 ++ (X, bind_sub typ_bot) :: E2) A.
Proof with auto.
  intros.
  assert (type A) as HA.
  get_type...
  ...
  -
    ...
    +
      rewrite_env ((X0 ~ bind_sub typ_bot ++ E1) ++ (X, bind_sub typ_bot) :: E2).
  ...
  -
    ...
    +
      rewrite_env ((X0 ~ bind_sub T1 ++ E1) ++ (X, bind_sub typ_bot) :: E2).
      apply H0 with (C:=C) (D:=D)...
      solve_notin.
...
Lemma WF_nominal_inversion: forall E1 E2 X A (X0:atom) D C,
    WF (X0 ~ bind_sub typ_bot ++
           map (subst_tb X (typ_label X (open_tt C X))) E1 ++ (X, bind_sub typ_bot) :: E2)
          (open_tt (subst_tt X (typ_label X (open_tt D X)) A) X0)->
    X \notin {{X0}} \u fl_tt A  ->
    wf_env (X0 ~ bind_sub typ_bot ++
           map (subst_tb X (typ_label X (open_tt C X))) E1 ++ (X, bind_sub typ_bot) :: E2) ->
    type (open_tt D X) ->
    WF (X0 ~ bind_sub typ_bot ++ E1 ++ (X, bind_sub typ_bot) :: E2) (open_tt A X0) .
Proof with auto.
  intros.
  rewrite subst_tt_open_tt_var in H...
  rewrite_env ((X0 ~ bind_sub typ_bot ++
           map (subst_tb X (typ_label X (open_tt C X))) E1) ++ (X, bind_sub typ_bot) :: E2) in H.
...
Lemma subst_reverse_equiv: forall A,
    ...
    equiv (map (subst_tb X (typ_label X (open_tt S X))) E1 ++ (X, bind_sub typ_bot) :: E2 ) (subst_tt X (typ_label X (open_tt C X)) A) (subst_tt X (typ_label X (open_tt D X)) B) ->
    equiv (E1 ++ (X, bind_sub typ_bot) :: E2) A B ->
    WF ((X, bind_sub typ_bot) :: E2) (open_tt C X) ->
    WF ((X, bind_sub typ_bot) :: E2) (open_tt D X) ->
    WF ((X, bind_sub typ_bot) :: E2) (open_tt S X) ->
    wf_env (E1 ++ (X, bind_sub typ_bot) :: E2) ->
    (equiv ((X, bind_sub typ_bot) ::E2) (open_tt C X)  (open_tt D X) \/ (X \notin fv_tt A \u fv_tt B )) .
Proof with auto.
  unfold equiv.
  ...
  -
    ...
    destruct H0 with (X:=Y) (X0:=X) (B:=open_tt T0 (typ_label Y (open_tt T0 Y))) (C:=C) (D:=D) (E1:=Y~bind_sub typ_bot ++E1) (E2:=E2) (S:=S)...
    +
      solve_notin.
    +
      split.
      *
        rewrite_env (Y ~ bind_sub typ_bot ++
                     map (subst_tb X (typ_label X (open_tt S X))) E1 ++ (X, bind_sub typ_bot) :: E2).
    ...
    +
      ...
      *
        apply H17...
    +      
      rewrite_env (Y ~ bind_sub typ_bot ++ E1 ++ (X, bind_sub typ_bot) :: E2)...
    +
      right.
  ...
  -    
    ...
    +
      ...
      *
        rewrite <- subst_tt_open_tt_var...
        rewrite <- subst_tt_open_tt_var...
        rewrite_env (nil ++ Y ~ bind_sub (subst_tt X (typ_label X (open_tt S X)) T1) ++ map (subst_tb X (typ_label X (open_tt S X))) E1 ++ (X, bind_sub typ_bot) :: E2).
        ...
        --
          ...
          rewrite H0.
          assert (equiv (X ~ bind_sub typ_bot ++ E2) (typ_label X (open_tt S X)) (typ_label X (open_tt S X))).
          {
    ...
    +
      split.
      *
        rewrite_env (nil ++ Y ~ bind_sub T1 ++ E1 ++ (X, bind_sub typ_bot) :: E2).
        apply sub_narrowing with (Q:=T0)...
        apply H5...
      *
        rewrite_env (Y ~ bind_sub T1 ++ E1 ++ (X, bind_sub typ_bot) :: E2).
        apply H6...        
    +
      rewrite_env (Y ~ bind_sub T1 ++ E1 ++ (X, bind_sub typ_bot) :: E2)...
      constructor...
  ...
  -
    clear IHHB1 IHHB2.
    assert (equiv (E1 ++ (X, bind_sub typ_bot) :: E2) (typ_rcd_cons i0 T0 T3)  (typ_rcd_cons i T1 T2)) as HE.
    { unfold equiv;split... }
    ...
    +
      solve_notin.
      apply lookup_some_in_fl_tt with (E:=(E1 ++ (X, bind_sub typ_bot) :: E2)) in H11...
      simpl in *.
    ...
    +
      ...
      *
        ...
        --
          ...
          apply lookup_some_subst with (E:=(E1 ++ (X, bind_sub typ_bot) :: E2))...
          get_well_form...
      *
    ...
    +
      ...
      *
        ...
        --
          ...
          apply fl_tt_dropLabel with (E:=(E1 ++ (X, bind_sub typ_bot) :: E2))...
          get_well_form...
      ...
      *
        ...
        --
          ...
          constructor...
          ++
            apply subst_tt_rt_type with (E:=E1 ++ (X, bind_sub typ_bot) :: E2)...
            rewrite_env (nil ++ E1 ++ (X, bind_sub typ_bot) :: E2).
            apply WF_weakening...
            get_well_form...
          ++
            apply subst_tt_rt_type with (E:=E1 ++ (X, bind_sub typ_bot) :: E2)...
            rewrite_env (nil ++ E1 ++ (X, bind_sub typ_bot) :: E2).
            apply WF_weakening...
            apply rt_type_drop with (E:=E1 ++ (X, bind_sub typ_bot) :: E2)...
            get_well_form...
            apply WF_drop...
            get_well_form...
          ++
            ...
            **
              ...
              rewrite subst_tt_dropLabel with (E:= E1 ++ (X, bind_sub typ_bot) :: E2)...
              simpl...
              ...
              dependent destruction H7...
            **
              assert ( WF (E1 ++ (X, bind_sub typ_bot) :: E2) T3).
              {
                get_well_form.
              ...
              rewrite union_swap_assoc...
              rewrite drop_collect_flip with (E:=E1 ++ (X, bind_sub typ_bot) :: E2) ...
              apply subst_tt_wf...
              rewrite_env (nil ++ E1 ++ (X, bind_sub typ_bot) :: E2).
              apply WF_weakening...
              apply subst_tt_rt_type with (E:=E1 ++ (X, bind_sub typ_bot) :: E2)...
              rewrite_env (nil ++ E1 ++ (X, bind_sub typ_bot) :: E2).
              apply WF_weakening...
              apply subst_tt_collectLabel_in with (E:=E1 ++ (X, bind_sub typ_bot) :: E2)...
              ...
              apply notin_drop_self...
          ++
            rewrite_env (nil ++ map (subst_tb X (typ_label X (open_tt S X))) E1 ++ (X, bind_sub typ_bot) :: E2).
            apply WF_narrowing_env with (S:=typ_label X (open_tt C X)).
            simpl.
            apply subst_tb_wf2 with (Q:=bind_sub typ_bot)...
            get_well_form...
          ++
            rewrite_env (nil ++ map (subst_tb X (typ_label X (open_tt S X))) E1 ++ (X, bind_sub typ_bot) :: E2).
          ...
          ++
            ...
            apply H8 with (i1:=i1)...
            **
              rewrite subst_tt_dropLabel with (E:=E1 ++ (X, bind_sub typ_bot) :: E2) in Ht...
              ...
              get_well_form...
            **
              rewrite subst_tt_dropLabel with (E:=E1 ++ (X, bind_sub typ_bot) :: E2) in H23...
        ...
        --
          ...
          ++
            ...
            **
              ...
              apply union_subset_x2 in H5...
              rewrite subst_tt_dropLabel with (E:= E1 ++ (X, bind_sub typ_bot) :: E2)...
              rewrite dropLable_notin with (E:= E1 ++ (X, bind_sub typ_bot) :: E2)...
              apply subst_tt_wf...
              rewrite_env (nil ++ E1 ++ (X, bind_sub typ_bot) :: E2).
              apply WF_weakening...
              apply subst_tt_rt_type with (E:= E1 ++ (X, bind_sub typ_bot) :: E2)...
              rewrite_env (nil ++ E1 ++ (X, bind_sub typ_bot) :: E2).
              apply WF_weakening...
              apply subst_tt_collect with (E:= E1 ++ (X, bind_sub typ_bot) :: E2)...
              apply subst_tt_collect with (E:= E1 ++ (X, bind_sub typ_bot) :: E2)...
            **
              assert ( WF (E1 ++ (X, bind_sub typ_bot) :: E2) T3).
              {
          ...
          ++
            ...
            apply H8 with (i1:=i1)...
            **
              rewrite subst_tt_dropLabel with (E:=E1 ++ (X, bind_sub typ_bot) :: E2) in H22...
      ...
      *
        ...
        --
          ...
          rewrite dropLable_notin  with (E:=E1 ++ (X, bind_sub typ_bot) :: E2) in H34...
        --
          simpl in *.
          assert (fv_tt T3 [=] fv_tt x \u fv_tt (dropLabel i T3)).
          {
            rewrite <- dropLabel_fv_tt with (E:=E1 ++ (X, bind_sub typ_bot) :: E2)...
            apply  KeySetProperties.equal_refl...
            get_well_form.
            dependent destruction H26...
          }
