...
Proof with auto.
  ...
  -
    ...
    +
      rewrite_env ((X ~ bind_sub typ_bot ++ F) ++ Z ~ bind_sub P ++ E).
      apply WF_narrowing with (V:=Q)...
      apply H3...
    +
      rewrite_env ((X ~ bind_sub typ_bot ++ F) ++ Z ~ bind_sub P ++ E).
...
Proof with auto.
  ...
  + apply WF_rec with (L:=L \u fv_tt A \u {{i}});intros.
    * unfold open_tt.
      ...
      apply H0...
    * rewrite subst_tt_intro with (X:=X)...
      ++ assert (WF (X ~ bind_sub typ_bot ++ E) (open_tt (dropLabel i A) X)).
        { unfold open_tt.
          rewrite open_tt_drop_var...
          apply H0... }
        apply subst_tt_wf...
...
(* Should use well-form, because
   record types relies on WF
    (i notin collecttypes ... )
*)

(* Lemma sub_transitivity : forall E Q,
  WF E Q ->
  forall S T,
  sub E S Q -> sub E Q T -> sub E S T.
Proof with auto.
...
Proof with auto.
  ...
  -
    ...
    + constructor...
      get_well_form...
    + clear IHsub. dependent induction H0...
      * apply sa_trans_tvar with (U:=U)...
      * apply sa_trans_tvar with (U:=U0)...
  ...
  -
    ...
    + constructor...
      get_well_form...
    + clear IHsub2. clear IHsub1. dependent induction H0...
      * apply sa_trans_tvar with (U:=U)...
        apply IHsub with (T4:=T1) (T3:=T2)...
      * inversion H0.
    + inversion H1.
  ...
  -
    ...
    + constructor...
      get_well_form...
    + clear H4 IHsub1 IHsub2.
      dependent induction H2...
      * apply sa_trans_tvar with (U:=U)...
        eapply IHsub with (T5:=T1) (T4:=T2)...
      * clear H2 IHsub1 IHsub2.
        apply sa_all with (L:=L \u L0 \u L1)...
        intros.
        apply H0 with (X:=X)...
  ...
  -
    ...
    + constructor...
      get_well_form...
    + clear H7. dependent induction H0...
      * apply sa_trans_tvar with (U:=U)...
      * inversion H1;subst... collect_nil H2.
  -
