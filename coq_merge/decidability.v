...
Fixpoint weight (T:typ) : nat :=
  ...
  | typ_and A B => 1 + weight A + weight B                                         
...
Lemma WFS_dec_aux: forall k E A,
    weight A <= k ->
    {WFS E A } + {~WFS E A}.
Proof with auto.
  induction k;intros...
  -
    induction A;simpl in *; try lia...
    +
      right...
      intros v.
      inversion v.
  ...
  -
    ...
    +
      ...
      *
        left.
        apply WFS_rec with (L:=fv_tt A \u dom E \u {{X}});intros...
        --
          rewrite subst_tt_intro with (X:=X0)...
          apply subst_tt_wfs with (X:=X0)...
          constructor...
...
Proof with auto.
  intros.
  apply WFS_dec_aux with (k:=weight A )...
Qed.
...
Fixpoint bindings_rec (E: menv) (n: nat) (T:typ) : nat :=
  ...
  | typ_arrow A B => S ( (bindings_rec E n A)+(bindings_rec E n B))
  | typ_and A B => S ( (bindings_rec E n A)+ (bindings_rec E n B))                     
...
Fixpoint close_tt_rec (K : nat) (Z : atom) (T : typ) {struct T} : typ :=
  ...
  | typ_and T1 T2 => typ_and (close_tt_rec K Z T1) (close_tt_rec K Z T2)
...
Lemma bindings_decompose_and: forall A B k,
    bindings (typ_and A B) <= S k ->
    bindings A <= k /\ bindings B <= k.
Proof with auto.
  intros.
  unfold bindings in *...
  simpl in *...
...
Lemma bindings_decompose_arrow: forall A B k,
    bindings (typ_arrow A B) <= S k ->
    bindings A <= k /\ bindings B <= k.
Proof with auto.
  intros.
  unfold bindings in *...
...
Lemma bindings_decompose_rcd: forall A B k,
    bindings (typ_rcd A B) <= S k ->
    bindings B <= k.
Proof with auto.
  intros.
  unfold bindings in *...
...
Lemma bindings_expand_and: forall A B ,
    bindings (typ_and A B) = 1 + bindings A + bindings B.
Proof with auto.
  intros.
  unfold bindings in *...
Qed.

Lemma bindings_expand_arrow: forall A B ,
    bindings (typ_arrow A B) = 1 + bindings A + bindings B.
Proof with auto.
  intros.
  unfold bindings in *...
Qed.

Lemma bindings_expand_rcd: forall A B ,
    bindings (typ_rcd A B) = 1 + bindings B.
Proof with auto.
  intros.
  unfold bindings in *...
Qed.

Lemma add_less: forall a b k,
     a + b <= k ->
    a <= k /\ b <= k.
Proof with auto.
  intros.
...
Lemma dec_toplike_aux : forall k A E,
    weight A <= k ->
    {toplike E A} + {~ toplike E A}.
Proof with auto.
  induction k.
  -
    induction A;intros;try solve [solve_right_dec|simpl in *;lia]...
    +
      pose wf_env_dec.
      destruct s with (E:=E)...
      right...
      ...
      inversion v...
  -
    destruct A;intros;try solve [solve_right_dec]...
    +
      pose wf_env_dec.
      destruct s with (E:=E)...
      right...
    ...
    +
      simpl in *.
      destruct IHk with (A:=A1) (E:=E);destruct IHk with (A:=A2) (E:=E);try lia...
      *
        right.
        intros v.
        inversion v...
    ...
    +
      ...
      simpl in *.
      pick fresh X.
      destruct IHk with (A:=open_tt A X) (E:=(X ~ bind_sub ++ E))...
      *
        rewrite weight_open_tt_equal...
        lia.
      *
        left.
        apply toplike_rec with (L:=fv_tt A \u {{X}} \u dom E)...
        intros.
        rewrite subst_tt_intro with (X:=X)...
    ...
    +
      simpl in *.
      pose WFS_dec.
      destruct s with (E:=E) (A:=A1).
      destruct IHk with (A:=A2) (E:=E);try lia...
      *
        right.
        intros v.
        inversion v...
    ...
    +
      simpl in *.
      destruct IHk with (A:=A) (E:=E);try lia...
      right.
      intros v.
      inversion v...
Qed.

Lemma toplike_dec : forall A E,
    {toplike E A} + {~ toplike E A}.
Proof with auto.
  intros.
  apply dec_toplike_aux with (k:=weight A)...
Qed.

Ltac solve_right_dec_wfe := right;intro v;get_well_form;auto.
Ltac solve_not_toplike :=
  match goal with
  | [H: toplike _ typ_bot |- _ ] => inversion H
  | [H: toplike _ typ_nat |- _ ] => inversion H
  | [H: toplike _ (typ_fvar _) |- _ ] => inversion H
  | [H: toplike _ (typ_bvar _) |- _ ] => inversion H
  | [H: WFS _ (typ_bvar _) |- _ ] => inversion H
  end.
Ltac solve_right_dec_toplike := right;intros v;dependent destruction v;solve_not_toplike.
Ltac solve_right_toplike E B:= pose toplike_dec as s;destruct s with (A:=B) (E:=E);auto;
                               right;intros v;apply topCorrectR in v;auto.
Ltac solve_right_and := right;intros v;apply and_inv in v;destruct_hypos;auto.
Ltac solve_right_not_toplike E B := pose toplike_dec as s;destruct s with (A:=B) (E:=E);try solve [left;apply S_top;auto|right;intros v;dependent destruction v;auto].

Lemma decidability_aux: forall k A B E ,
     (bindings A) + (bindings B) <= k ->
    {Sub E A B} + {~ Sub E A B}.
Proof with auto.
  intros k.
  induction k.
  -
    intros A;induction A;intros B;induction B;intros;try solve [solve_top_dec E;solve_right_dec_wfe|solve_top_dec E;solve_right_dec_toplike|unfold bindings in *;simpl in *;lia]...
    +
      solve_top_dec E.
    ...
    +
      ...
      *
        ...
        inversion H1.
  -
    intros A;destruct A;intros B;destruct B;intros;try solve [solve_top_dec E;solve_right_dec_wfe|solve_top_dec E;solve_right_dec_toplike|unfold bindings in *;simpl in *;lia]...
    +
      solve_right_toplike E (typ_and B1 B2).
    +
      solve_right_toplike E (typ_mu B).
    +
      solve_right_toplike E (typ_arrow B1 B2).
    +
      solve_right_toplike E (typ_rcd a B).
    +
      solve_top_dec E.
      solve_top_wfs_dec E a...
      solve_right_dec_wfe.
    ...
    +
      solve_top_dec E...
      solve_top_wfs_dec E (typ_and B1 B2)...
      solve_right_dec_wfe.
      solve_right_dec_wfe.
    +
      solve_top_dec E...
      solve_top_wfs_dec E (typ_mu B)...
      solve_right_dec_wfe.
      solve_right_dec_wfe.
    +
      solve_top_dec E...
      solve_top_wfs_dec E (typ_arrow B1 B2)...
      solve_right_dec_wfe.
      solve_right_dec_wfe.
    +
      solve_top_dec E...
      solve_top_wfs_dec E (typ_rcd a B)...
      solve_right_dec_wfe.
      solve_right_dec_wfe.
    +
      simpl in H.
      apply bindings_decompose_and in H.
      destruct_hypos.
      destruct IHk with (E:=E) (A:=typ_nat) (B:=B1);destruct IHk with (E:=E) (A:=typ_nat) (B:=B2);try solve [auto|solve_right_and]...
    +
      solve_right_not_toplike E (typ_mu B).
    +
      solve_right_not_toplike E (typ_arrow B1 B2).
    +
      solve_right_not_toplike E (typ_rcd a B).
    +
      solve_right_dec_wfe.
      inversion H1.
    +
    ...
    +
      apply add_less in H.
      destruct_hypos.
      apply bindings_decompose_and in H0.
      destruct_hypos.
      destruct IHk with (E:=E) (A:=a) (B:=B1);destruct IHk with (E:=E) (A:=a) (B:=B2);try solve [auto|solve_right_and]...
    +
      solve_top_wfs_dec E a...
      solve_right_not_toplike E (typ_mu B).
    +
    ...
    +
      solve_top_wfs_dec E a...
      solve_right_not_toplike E (typ_rcd a0 B).
    +
      apply add_less in H.
      destruct_hypos.
      apply bindings_decompose_and in H.
      destruct_hypos.
      solve_top_dec E.
      solve_top_wfs_dec E (typ_and A1 A2).
      destruct IHk with (E:=E) (A:=typ_top) (B:=A1);destruct IHk with (E:=E) (A:=typ_top) (B:=A2);try solve [auto|solve_right_dec]...
      right.
      intros v.
      get_well_form...
       right.
    ...
    +
      ...
      destruct_hypos.
      solve_top_dec E.
      solve_top_wfs_dec E A1.
      rename Qt into Qp.
      solve_top_wfs_dec E A2.
      assert (bindings typ_bot = 0). unfold bindings in *...
      destruct IHk with (E:=E) (B:=typ_bot) (A:=A1);destruct IHk with (E:=E) (B:=typ_bot) (A:=A2);try solve [auto|solve_right_dec];try rewrite H2;try lia...
      right...
      intros v.
      get_well_form...
    ...
    +
      ...
      rename Qt into Qp.
      solve_top_wfs_dec E A2.
      assert (bindings typ_nat = 0). unfold bindings in *...
      destruct IHk with (E:=E) (B:=typ_nat) (A:=A1);destruct IHk with (E:=E) (B:=typ_nat) (A:=A2);try solve [auto|solve_right_dec];try rewrite H2;try lia...
      right...
      intros v.
      get_well_form...
      dependent destruction H3...
    ...
    +
      ...
      rename Qt into Qp.
      solve_top_wfs_dec E A2.
      assert (bindings a = 0). unfold bindings in *...
      destruct IHk with (E:=E) (B:=a) (A:=A1);destruct IHk with (E:=E) (B:=a) (A:=A2);try solve [auto|solve_right_dec];try rewrite H2;try lia...
      right...
      intros v.
      get_well_form...
      dependent destruction H3...
    ...
    +
      solve_top_dec E.
      solve_top_wfs_dec E B1.
      rename Qt into Q1.
      solve_top_wfs_dec E B2.
      rename Qt into Q2.
      solve_top_wfs_dec E A1.
      rename Qt into Q3.
      solve_top_wfs_dec E A2.
      rename Qt into Q4.
      *
        destruct IHk with (A:=typ_and A1 A2) (B:=B1) (E:=E)...
        rewrite bindings_expand_and in *.
        rewrite bindings_expand_and in *.
        lia.
        destruct IHk with (A:=typ_and A1 A2) (B:=B2) (E:=E)...
        rewrite bindings_expand_and in *.
        rewrite bindings_expand_and in *.
        lia.
        right.
    ...
    +
      ...
      rename Qt into Q3.
      *
        destruct IHk with (A:=A1) (B:=typ_mu B) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        destruct IHk with (A:=A2) (B:=typ_mu B) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        right.
        intros v.
    ...
    +
      ...
      rename Qt into Q3.
      *
        destruct IHk with (A:=A1) (B:=typ_arrow B1 B2) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        destruct IHk with (A:=A2) (B:=typ_arrow B1 B2) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        right.
        intros v.
    ...
    +
      ...
      rename Qt into Q3.
      *
        destruct IHk with (A:=A1) (B:=typ_rcd a B) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        destruct IHk with (A:=A2) (B:=typ_rcd a B) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        right.
        intros v.
    ...
    +
      ...
      rename Qt into Q3.
      *
        destruct IHk with (A:=typ_mu A) (B:=B1) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        destruct IHk with (A:=typ_mu A) (B:=B2) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        right.
        intros v.
    ...
    +
      ...
      *
        left.
        apply S_top...
        apply WFS_rec with (L:={{X}} \u fv_tt A);intros...
        --
          rewrite subst_tt_intro with (X:=X0)...
          apply subst_tt_wfs...
          constructor...
    ...
    +
      ...
      rename Qt into Q3.
      *
        destruct IHk with (A:=typ_arrow A1 A2) (B:=B1) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        destruct IHk with (A:=typ_arrow A1 A2) (B:=B2) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        right.
        intros v.
    ...
    +
      ...
      solve_right_dec_wfe.
    +
      rewrite bindings_expand_arrow in *.
      rewrite bindings_expand_arrow in *.
      solve_right_not_toplike E (typ_arrow B1 B2).
      *
        solve_top_dec E...
    ...
    +
      ...
      rename Qt into Q3.
      *
        destruct IHk with (A:=typ_rcd a A) (B:=B1) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        destruct IHk with (A:=typ_rcd a A) (B:=B2) (E:=E)...
        rewrite bindings_expand_and in *.
        lia.
        right.
        intros v.
    ...
    +
      ...
      solve_right_dec_wfe.
    +
      rewrite bindings_expand_rcd in *.
      rewrite bindings_expand_rcd in *.
      solve_right_not_toplike E (typ_rcd a0 B).
      *
        solve_top_dec E...
...
Proof with auto.
  intros.
  apply decidability_aux with (k:= (bindings A) + (bindings B))...
Qed.
